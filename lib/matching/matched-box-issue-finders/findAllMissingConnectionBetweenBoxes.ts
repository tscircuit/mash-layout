...
export function findAllMissingConnectionBetweenBoxes(params: {
  ...
  for (let pinNumber = 1; pinNumber <= targetPinCount; pinNumber++) {
    ...
    const connectionsWithThisPin = targetInputNetlist.connections.filter(
      (conn) =>
        conn.connectedPorts.some(
          (port) =>
            "boxId" in port &&
            port.boxId === targetBox.boxId &&
    ...
    for (const connection of connectionsWithThisPin) {
      ...
      for (const port of connection.connectedPorts) {
        if ("boxId" in port && port.boxId !== targetBox.boxId) {
          ...
          const otherTargetBoxIndex = targetInputNetlist.boxes.findIndex(
          ...
          const candidateOtherBox =
            candidateInputNetlist.boxes[
              matchedBoxForOtherTarget.candidateBoxIndex
            ]!

          // Look for a connection between the candidate boxes at the same pin positions
          const candidateConnectionExists =
            candidateInputNetlist.connections.some(
              (candidateConn) =>
                candidateConn.connectedPorts.some(
                  (candidatePort) =>
