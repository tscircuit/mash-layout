...
test("corpus01 - template matching and basic structure", async () => {
  ...
  const { solver, laidOutSchematicSvg, laidOutCircuitJson } = await testTscircuitCodeForLayout(
    corpus1Code,
    {
  ...
  const resistor = laidOutCircuitJson.find(
    (c: any) =>
      c.type === "schematic_component" &&
      c.source_component_id === "source_component_1",
  )
  expect(resistor?.symbol_name).toContain("up")
